...
const PeriodSelector = forwardRef((props, ref) => {
  ...
  const handleChange = (slug) => {
    ...
    if (slug == "current_financial_year") {
      ...
      if (currentMonth > 5) {
        ...
        setCurrentPeriod({
          from: dayjs(`${dayjs().subtract(1, "year").year()}-07-01`).format(
            "YYYY-MM-DD"
          ),
    ...
    if (slug == "last_financial_year") {
      setCurrentPeriod({
        from: dayjs(`${dayjs().subtract(2, "year").year()}-07-01`).format(
          "YYYY-MM-DD"
        ),
  ...
  useEffect(() => {
    console.log("Length-------", Object.keys(props).length);
    ...
    default:
      return (
        <>
          ...
          >
            <Icon sx={{ color: blue[600] }}>date_range</Icon>
          </IconButton>
          <Dialog
            open={open}
                    ...
                    >
                      <List dense disablePadding>
                        {periods.map((period) => (
                          ...
                          >
                            <ListItemButton
                              onClick={() => handleChange(period.slug)}
                            >
                              <ListItemText children={period.name} />
                            </ListItemButton>
                          </ListItem>
                        ))}
                      </List>
          ...
          <Box>
            <Box border={1} borderColor="divider" p={2}>
              ...
              >
                ...
                {_.has(props, "periodUpto") && props.periodUpto && (
                  <Grid item xs={12} md={4}>
                    <DatePicker
                      date={currentperiod.to}
                      setDate={(value) =>
